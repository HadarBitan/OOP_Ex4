...
class GraphAlgo:
    ...
    def load_from_json(self, graph_json: str):
        graph_out = json.loads(graph_json, object_hook=lambda json_dict: SimpleNamespace(**json_dict))
        graph = DiGraph()
        for n in graph_out.Nodes:
            x, y, _ = n.pos.split(',')
            n.pos = SimpleNamespace(x=float(x), y=float(y))
            id_ = n.id
            graph.add_node(id_, (float(x), float(y), 0.0))
        for e in graph_out.Edges:
            src = e.src
            dest = e.dest
            w = e.w
            graph.add_edge(src, dest, float(w))
        self.__graph = graph
    ...
    def get_pos(self,id):
        for i,n in self.get_graph().get_all_v().items():
            if i==id:
