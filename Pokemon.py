...
class Pokemon:
    ...
    """
    def __init__(self, value: float, poke_type: int, pos: tuple, graph):
    ...
    def _find_edge(self):
        ...
        """
        for edge in self._graph.get_graph().get_all_e().keys():
            srcPos = self._graph.get_graph().get_all_v()[edge[0]].get_location()
            destPos = self._graph.get_graph().get_all_v()[edge[1]].get_location()
